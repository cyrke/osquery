...
class KafkaBgPoller : public InternalRunnable {
 public:
  KafkaBgPoller(KafkaProducerPlugin* kpp, size_t timeout)
      : kpp_(kpp), timeout_(timeout * 1000) {}

 public:
  // Kafka background poller entry point.
  void start() override;

 private:
  KafkaProducerPlugin* kpp_;
  std::chrono::milliseconds timeout_;
};

REGISTER(KafkaProducerPlugin, "logger", "kafka_producer");

void KafkaBgPoller::start() {
  while (kpp_->running_.load()) {
    pauseMilli(timeout_);
    if (interrupted()) {
      kpp_->stop();
      break;
